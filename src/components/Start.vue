  ...
  <div class="app-container">
    ...
    <div class="content-container">
      ...
      <div v-else-if="currentStep === 'start'" class="start-survey-container">
        <h2>
          Bonjour,<br />
          Dans le cadre d'une étude pour <br />SNCF Gares & Connexions,<br /> auriez-vous une
          minute pour répondre à deux questions? 
        </h2>
...
const surveyCollectionRef = collection(db, "Nantes");
const counterDocRef = doc(db, "counterNantes", "surveyCounter");

// Computed properties
const currentQuestion = computed(() => {
  return currentQuestionIndex.value >= 0 &&
    currentQuestionIndex.value < questionsData.value.length
...
const selectAnswer = (option) => {
  if (currentQuestion.value) {
    answers.value[currentQuestion.value.id] = option.id.toString();

    if (option.next === "end") {
      finishSurvey();
    } else if (option.freeText) {
      // If the selected option has freeText, move to the next question (which should be the free text input)
...
const handleFreeTextAnswer = () => {
  if (currentQuestion.value) {
    ...
    if (currentQuestion.value.next === "end") {
      ...
      nextQuestion(currentQuestion.value.next);
    }
  }
};
...
const nextQuestion = (forcedNextId = null) => {
  ...
  if (nextQuestionId === "end") {
    ...
    const nextIndex = questionsData.value.findIndex(
...
const previousQuestion = () => {
  if (canGoBack.value) {
    ...
    const previousIndex = questionsData.value.findIndex(
...
const finishSurvey = async () => {
  ...
  // Add answers to surveyData, using null for unanswered questions
  questions.forEach(question => {
    surveyData[question.id] = answers.value[question.id] ?? null;
  });
...
const getNextId = async () => {
  ...
  return `Nantes-${counter.toString().padStart(6, "0")}`;
};
