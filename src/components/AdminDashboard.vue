...
const downloadData = async () => {
  try {
    ...
    const data = querySnapshot.docs.map(doc => {
      ...
      return headerOrder.reduce((acc, key) => {
        if (['ID_questionnaire', 'ENQUETEUR', 'DATE', 'JOUR', 'HEURE_DEBUT', 'HEURE_FIN'].includes(key)) {
          acc[key] = docData[key]?.toString() || null;
        } else {
          const question = questions.find(q => q.id === key);
          if (question) {
            if (question.options) {
              const selectedOption = question.options.find(opt => opt.id.toString() === docData[key]);
              acc[key] = selectedOption ? selectedOption.id.toString() : null;
            } else if (question.freeText) {
              acc[key] = docData[key] || null;
            } else {
              acc[key] = null;
            }
          } else {
    ...
    XLSX.writeFile(workbook, `Nantes_${timestamp}.xlsx`);
